module Fog
  module AWS
    class Elasticsearch < Fog::Service
      ...
      request_path 'fog/aws/requests/elasticsearch'
      request :list_domain_names
...
#                :event_source_mappings => {}
      ...
      class Real
        ...
        #
        # ==== Returns
        # * Elasticsearch object with connection to AWS.
        def initialize(options={})
          @use_iam_profile    = options[:use_iam_profile]
          @connection_options = options[:connection_options] || {}
          @instrumentor       = options[:instrumentor]
          @instrumentor_name  = options[:instrumentor_name] || 'fog.aws.elasticsearch'

          options[:region] ||= 'us-east-1'
          @region = options[:region]
          @host = options[:host] || "es.#{options[:region]}.amazonaws.com"

          @path       = options[:path]        || '/'
          @persistent = options[:persistent]  || false
          @port       = options[:port]        || 443
          @scheme     = options[:scheme]      || 'https'
          @version    = options[:version]     || '2015-01-01'
        ...
        def setup_credentials(options)
          ...
          @signer = Fog::AWS::SignatureV4.new( @aws_access_key_id, @aws_secret_access_key, @region, 'es')
        end
...
#              :path              => request_path_to_sign,
        ...
        def _request(method, path, query, body, headers, expects, idempotent, parser=nil)
          ...
          raise Fog::AWS::Elasticsearch::Error.slurp(error,
